...
// ============ 🏪 POS SYSTEM - ADD THIS ENTIRE BLOCK HERE ============
let currentCashItems = [];
let allBooks = [];

// Load books for POS
async function loadBooksForPOS() {
    try {
        const token = localStorage.getItem('token');
        ...
        const select = document.getElementById('cashBookSelect');
        if (!select) return;
        
        select.innerHTML = '<option value="">-- Search and Select Book --</option>';
        
        allBooks.forEach(book => {
            const option = document.createElement('option');
            option.value = book._id;
            option.textContent = `${book.title} | ₹${book.price} | Stock: ${book.stock}`;
            option.dataset.title = book.title;
            option.dataset.price = book.price;
            option.dataset.stock = book.stock;
            option.dataset.originalPrice = book.originalPrice;
        ...
        // Show stock info when selecting a book
        select.addEventListener('change', function() {
            const selected = this.options[this.selectedIndex];
            const stockInfo = document.getElementById('cashStockInfo');
            const unitPrice = document.getElementById('cashUnitPrice');
            
            if (selected.value && unitPrice && stockInfo) {
                unitPrice.value = selected.dataset.price;
                stockInfo.innerHTML = `<span style="color: ${selected.dataset.stock < 10 ? 'red' : 'green'}">
                     📚 Stock available: ${selected.dataset.stock}
                    </span>`;
            } else if (unitPrice && stockInfo) {
                unitPrice.value = '';
                stockInfo.innerHTML = '';
            }
        });
...
function addCashItem() {
    ...
    const quantity = parseInt(document.getElementById('cashQuantity')?.value || '1');
    const price = parseFloat(document.getElementById('cashUnitPrice')?.value || '0');
    ...
    const existingIndex = currentCashItems.findIndex(item => item.bookId === bookId);
    if (existingIndex !== -1) {
        const newQty = currentCashItems[existingIndex].quantity + quantity;
        if (newQty > maxStock) {
            alert(`Cannot add ${quantity}. Only ${maxStock - currentCashItems[existingIndex].quantity} more available.`);
            return;
        }
        currentCashItems[existingIndex].quantity = newQty;
        currentCashItems[existingIndex].total = newQty * price;
    } else {
        currentCashItems.push({
            bookId: bookId,
            title: title,
            quantity: quantity,
            price: price,
    ...
    updateCashCartDisplay();
    
    // Reset fields
    const qtyInput = document.getElementById('cashQuantity');
...
// Update cart display and recalculate totals
function updateCashCartDisplay() {
    const tbody = document.getElementById('cashCartBody');
    if (!tbody) return;
    
    tbody.innerHTML = '';
    let subtotal = 0;
    
    currentCashItems.forEach((item, index) => {
        subtotal += item.total;
        tbody.innerHTML += `
            <tr>
                ...
                <td style="padding: 8px; text-align: center; border-bottom: 1px solid #ddd;">
                    <button onclick="updateQuantity(${index}, -1)" style="padding: 0 5px;">-</button>
                    ${item.quantity}
                    <button onclick="updateQuantity(${index}, 1)" style="padding: 0 5px;">+</button>
                </td>
                ...
                <td style="padding: 8px; text-align: center; border-bottom: 1px solid #ddd;">
                    <button onclick="removeCashItem(${index})" style="color: red;">❌</button>
                </td>
            </tr>
        `;
    });
    
    const subtotalInput = document.getElementById('cashSubtotal');
...
function updateQuantity(index, delta) {
    if (!currentCashItems[index]) return;
    
    const newQty = currentCashItems[index].quantity + delta;
    if (newQty < 1) {
        removeCashItem(index);
        return;
    }
    
    // Get max stock
    const book = allBooks.find(b => b._id === currentCashItems[index].bookId);
    if (book && newQty > book.stock) {
        alert(`Only ${book.stock} available!`);
        return;
    }
    
    currentCashItems[index].quantity = newQty;
    currentCashItems[index].total = newQty * currentCashItems[index].price;
    updateCashCartDisplay();
}

// Remove item
function removeCashItem(index) {
    currentCashItems.splice(index, 1);
    updateCashCartDisplay();
}
...
// Calculate grand total with manual discount and shipping
function calculateGrandTotal() {
    const subtotalInput = document.getElementById('cashSubtotal');
    const discountInput = document.getElementById('cashDiscount');
    const discountType = document.getElementById('cashDiscountType');
    const shippingInput = document.getElementById('cashShipping');
    const grandTotalSpan = document.getElementById('cashGrandTotal');
    
    if (!subtotalInput) return;
    
    const subtotal = parseFloat(subtotalInput.value) || 0;
    let discountAmount = parseFloat(discountInput?.value) || 0;
    const discountTypeValue = discountType?.value || 'fixed';
    const shipping = parseFloat(shippingInput?.value) || 0;
    
    // Calculate discount
    let actualDiscount = discountAmount;
    if (discountTypeValue === 'percent') {
        actualDiscount = (discountAmount / 100) * subtotal;
    }
    
    const grandTotal = subtotal - actualDiscount + shipping;
    if (grandTotalSpan) grandTotalSpan.innerText = Math.round(grandTotal);
}

// Process cash sale
async function processCashSale() {
    if (currentCashItems.length === 0) {
        alert('No items in sale!');
        return;
    }
    
    const customerName = document.getElementById('cashCustomerName')?.value.trim() || 'Walk-in Customer';
    const paymentMethod = document.getElementById('cashPaymentMethod')?.value || 'cash';
    const notes = document.getElementById('cashNotes')?.value || '';
    
    // Calculate final totals
    const subtotal = parseFloat(document.getElementById('cashSubtotal')?.value) || 0;
    let discountAmount = parseFloat(document.getElementById('cashDiscount')?.value) || 0;
    const discountType = document.getElementById('cashDiscountType')?.value || 'fixed';
    const shipping = parseFloat(document.getElementById('cashShipping')?.value) || 0;
    ...
    const saleData = {
        ...
        items: currentCashItems.map(item => ({
            id: item.bookId,
            title: item.title,
        ...
        totals: {
            ...
            discountFixed: discountType === 'fixed' ? discountAmount : 0,
            shipping: shipping,
            tax: 0,
            total: grandTotal
        },
    ...
    try {
        ...
        if (result.success) {
            ...
            currentCashItems = [];
            updateCashCartDisplay();
            const customerNameInput = document.getElementById('cashCustomerName');
            const discountInput = document.getElementById('cashDiscount');
            const shippingInput = document.getElementById('cashShipping');
            const notesInput = document.getElementById('cashNotes');
            const paymentSelect = document.getElementById('cashPaymentMethod');
...
function printReceipt(data) {
    ...
    receiptWindow.document.write(`
        <html>
        <head><title>Receipt - ${data.orderId}</title></head>
        <body style="font-family: monospace; padding: 20px;">
            <h2 style="text-align: center;">Shastraprathista</h2>
            ...
            ${data.totals.discount > 0 ? `<p><strong>Discount:</strong> -₹${data.totals.discount}</p>` : ''}
            ${data.totals.shipping > 0 ? `<p><strong>Shipping:</strong> ₹${data.totals.shipping}</p>` : ''}
            <p><strong>Total:</strong> ₹${data.totals.total}</p>
...
// Add event listeners for manual adjustments (after DOM ready)
...
window.removeCashItem = removeCashItem;
window.clearCashCart = clearCashCart;
window.calculateGrandTotal = calculateGrandTotal;
window.processCashSale = processCashSale;
