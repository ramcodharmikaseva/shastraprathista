...
router.get('/test-email-simple', async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: '✅ Email Test from Shastraprathista',
    ...
    let advice = '';
    if (error.code === 'EAUTH') {
      advice = 'Authentication failed. Please: 1) Enable 2FA on Gmail, 2) Generate App Password, 3) Use App Password (16 chars without spaces)';
    } else if (error.code === 'EENVELOPE') {
      advice = 'Email address issue. Check FROM/TO addresses.';
    } else {
      advice = 'Check your internet connection and email configuration.';
    }
...
// ✅ TEST EMAIL CONFIGURATION - FIXED ROUTE
router.get('/test/email', async (req, res) => {
  try {
    ...
    // Test with a simple order object
    const testOrder = {
      orderId: 'TEST123',
...
router.get('/test/email', async (req, res) => {
  try {
    const testOrder = {
      ...
      customerName: 'Test Customer',
      customerEmail: 'shastraprathista@gmail.com',
      customerPhone: '9876543210',
      billingAddress: {
        fullName: 'Test Customer',
        addressLine1: '123 Test Street',
    ...
    const results = {
      confirmation: await sendOrderConfirmationEmail(testOrder),
      shipped: await sendOrderShippedEmail(testOrder),
      delivered: await sendOrderDeliveredEmail(testOrder),
      admin: await sendAdminOrderNotification(testOrder)
    };
